...
from GDTS import GDTS
import time 

class GDTS1:
    def __init__(self, turn_rate, turn_radius = 10, strength=100):
    ...
    def initialization(self):
        ...
        self.threshold_dist = self.turn_radius /2   # self.turn_radius
        self.uav_pos = []
    ...
    def iter_loop(self):
        ...
        def custom_objects():
            return {'mean_squared_error': MeanSquaredError()}

        model = load_model("D://GaussianPredictionModel2.h5", custom_objects=custom_objects())
        ...
        while curr_dist > self.threshold_dist:
            ...
            if self.loop == 1:
                if self.timestep > 2 and (self.strengths[self.timestep - 2] < self.strengths[self.timestep - 1] and self.strengths[self.timestep - 1] > self.strengths[self.timestep]):
                    ...
                    # self.gradient_direction(self.uav_pos, self.strengths)

                    self.grad_dir = model.predict(np.array([x , y , self.strengths[-1]]).reshape(1,-1))
                ...
                angle_diff = np.degrees(np.arccos(np.dot(self.p, self.grad_dir.reshape(2)) / (np.linalg.norm(self.p) * np.linalg.norm(self.grad_dir))))
                if abs(angle_diff) < self.delta:
                    ...
                    theta = np.degrees(np.arctan2(self.grad_dir[0][1], self.grad_dir[0][0]))
    ...
    def plotting_path(self):
        ...
        gt = GDTS(0.5) 
        x_c = gt.total_path_coordinates_x
        y_c = gt.total_path_coordinates_y

        plt.figure(figsize=(10, 10))
        plt.scatter(self.total_path_coordinates_x, self.total_path_coordinates_y, c='r', s=2, label='DL')
        plt.scatter(x_c , y_c ,c='g', s=2, label='GDTS' )
